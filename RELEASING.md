...
6. Run `bash tests/smoke.sh` — must print PASS.
7. Commit with a conventional message describing the change.
8. Tag: `git tag v<version> && git push origin main --tags`.
9. (Optional) Verify a fresh install: `uv tool install --force git+https://github.com/cheapsteak/agent-channels` then `channels --help`.
